...
<div id="contner">	
		...
		include ("personne.php");
			...
			?>
					...
					<?php
						...
						$req = $bdd->query('SELECT Article, Design, Qtite, Vendeur,R_vendeur, Acheteur,R_acheteur, Type_Op, Utilisateur, DATE_FORMAT(Date, \'%d/%m/%Y à %Hh%imin%ss\') AS date_fr FROM ces_int WHERE R_acheteur = "'.$ray.'"  AND Date >= "'.$_GET['date1'].'"  AND Date <= "'.$_GET['date2'].'" ORDER BY Date DESC ')or die(print_r($bdd->errorInfo()));
					}else{
						
						$req = $bdd->query('SELECT Article, Design, Qtite, Vendeur,R_vendeur, Acheteur,R_acheteur, Type_Op, Utilisateur, DATE_FORMAT(Date, \'%d/%m/%Y à %Hh%imin%ss\') AS date_fr FROM ces_int WHERE R_acheteur = "'.$ray.'"  AND Utilisateur="'.$user.'" AND Date >= "'.$_GET['date1'].'"  AND Date <= "'.$_GET['date2'].'" ORDER BY Date DESC ')or die(print_r($bdd->errorInfo()));
	...
	<form action="convert.php?date1=<?php echo $date_d;?>&date2=<?php echo $date_f;?>&user=<?php echo $user;?>&rayon=<?php echo $ray;?>" method="POST">
