...
$date_j=date('jmYHi');
	...
	extract($_POST);
	{
		...
		$rep=$bdd->query('SELECT COUNT(*)AS nb from ces_tempo  ');
		$donnees=$rep->fetch();	
		$nb_news=$donnees['nb'];
		if ($nb_news==0)
		{
		...
		$sql = mysql_query('SELECT Article, Design, Qtite, Vendeur,R_vendeur, Acheteur,R_acheteur, Type_Op, Utilisateur, DATE_FORMAT(Date, \'%d/%m/%Y \') AS Date_fr FROM ces_tempo  ');
		}else{
		
		$sql = mysql_query('SELECT Article, Design, Qtite, Vendeur,R_vendeur, Acheteur,R_acheteur, Type_Op, Utilisateur, DATE_FORMAT(Date, \'%d/%m/%Y \') AS Date_fr FROM ces_tempo WHERE  Utilisateur="'.$user.'"  ');
