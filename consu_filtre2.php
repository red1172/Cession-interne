		...
		include ("personne.php");
			...
			?>
					...
					<?php
						...
						$req = $bdd->query('SELECT Article, Design, Qtite, Vendeur,R_vendeur, Acheteur,R_acheteur, Type_Op, Utilisateur, DATE_FORMAT(Date, \'%d/%m/%Y à %Hh%imin%ss\') AS date_fr FROM ces_int WHERE Date >= "'.$date_d.'"  AND Date <= "'.$date_f.'"  ORDER BY Date DESC ')or die(print_r($bdd->errorInfo()));
					}else{
						
						$req = $bdd->query('SELECT Article, Design, Qtite, Vendeur,R_vendeur, Acheteur,R_acheteur, Type_Op, Utilisateur, DATE_FORMAT(Date, \'%d/%m/%Y à %Hh%imin%ss\') AS date_fr FROM ces_int WHERE Date >= "'.$date_d.'"  AND Date <= "'.$date_f.'"  AND Utilisateur="'.$user.'" ORDER BY Date DESC ')or die(print_r($bdd->errorInfo()));
						
						}
			?>
		
		
			<form action="consu_filtre_ray2.php?date1=<?php echo $date_d;?>&date2=<?php echo $date_f;?>" method="POST">
				<label>Rayon : </label>
				<select name="ray">
