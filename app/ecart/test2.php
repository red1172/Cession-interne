...
$stid = oci_parse($conn, "Select fin_mtcabrut from hyp010.MGFIN  where FIN_DTREM='24-05-2013' ");
oci_execute($stid);
if (ocifetch($stid)){
...
//CA ENCAI METI
$stid = oci_parse($conn, "Select Sum( ECS_MTTHEOEC)  from hyp010.MGECS where ECS_DTREM='24-05-2013' ");
oci_execute($stid);
if (ocifetch($stid)){
...
// ESPECES METI
$stid = oci_parse($conn, "Select  ECS_CDTCHCAI as type_enc , ECS_MTTHEOEC as theorique, ECS_MTREELEC as real  from hyp010.MGECS where ECS_DTREM='24-05-2013' 
 and  ECS_CDTCHCAI=1 ");
oci_execute($stid);
if (ocifetch($stid)){
...
// Retour METI
$stid = oci_parse($conn, "Select sum(VUG_MTRETOUR) from MGVUG where VUG_DTREM='24-05-2013' ");
oci_execute($stid);
if (ocifetch($stid)){
...
$stid = oci_parse($conn, "Select  ECS_CDTCHCAI as type_enc , ECS_MTTHEOEC as theorique, 
 ECS_MTREELEC as real  from hyp010.MGECS where ECS_DTREM='24-05-2013' and 
 ECS_CDTCHCAI=5");
oci_execute($stid);
if (ocifetch($stid)){
...
$tab3 = $date_hier_tplinux.'ejmedia';
echo $tab;
//CA TPLINUX
$sql ="  Select sum(amtsold) from bd_2013052400.plumove where period=1 ";
...
//ESPECE TPLINUX
$sql ="select medcurrdesc,  sum(CASE WHEN substring(flag from 3 for 1)='1' THEN   mediaamnt*-1 ELSE mediaamnt END) as amount,  mediatype from bd_2013052400.ejmedia left join media on (mediatype=medcurrnmbr)
...
//RETOUR TPLINUX
 $sql="Select sum(amtsold) from bd_2013052400.ejitem_return";
$result = pg_query($dbconntplinux,$sql);
$rows = pg_num_rows($result);
...
//CHEQUE TPLINUX
$sql="select sum(mediaamnt) from bd_2013052400.ejmedia where mediatype='28'";
$result = pg_query($dbconntplinux,$sql);
$rows = pg_num_rows($result);
...
//BON DIVERS TPLINUX
$sql="select sum(mediaamnt) from bd_2013052400.ejmedia where mediatype='12'";
$result = pg_query($dbconntplinux,$sql);
$rows= pg_num_rows($result);
...
$sql="select sum(mediaamnt) from bd_2013052400.ejmedia where mediatype='14'";
...
$sql="select sum(mediaamnt) from bd_2013052400.ejmedia where mediatype='7'";
$result = pg_query($dbconntplinux,$sql);
$rows = pg_num_rows($result);
