...
$date_j=date('jmYHi');
	...
	extract($_POST);
	{
		...
		$sql = mysql_query('SELECT  Article, Design as Désignation, Qtite, Vendeur,R_vendeur, Acheteur,R_acheteur, Type_Op, Utilisateur, DATE_FORMAT(Date, \'%d/%m/%Y \') AS DATE, TIME(Heure)as HEURE FROM ces_int WHERE Date >= "'.$date_d.'"  AND Date <= "'.$date_f.'"  AND R_acheteur = "'.$ray.'" ORDER BY Date DESC ');
		}else{
		
		$sql = mysql_query('SELECT  Article, Design as Désignation, Qtite, Vendeur,R_vendeur, Acheteur,R_acheteur, Type_Op, Utilisateur, DATE_FORMAT(Date, \'%d/%m/%Y \') AS DATE, TIME(Heure)as HEURE  FROM ces_int WHERE Date >= "'.$date_d.'"  AND Date <= "'.$date_f.'"  AND R_acheteur = "'.$ray.'" AND Utilisateur="'.$user.'" ORDER BY Date DESC ');
