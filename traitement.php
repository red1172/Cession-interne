...
$h = CURRENT_TIMESTAMP(); 
$req=$bdd->prepare("INSERT INTO ces_int(Article, Design, Qtite, Vendeur,R_vendeur, Acheteur,R_acheteur, Type_Op, Utilisateur, Heure, Date) VALUES(:art, :des, :qtite, :n_vd, :r_vd, :n_acht, :r_acht,  :typ_op, :user, :h, now())") or die(print_r($bdd->errorInfo()));
$req->execute(array('art'=>$article,  'des'=>$desig,'n_vd'=>$n_vd, 'r_vd'=>$r_vd, 'n_acht'=>$nom_r_acht, 'r_acht'=>$r_acht,  'qtite'=>$qtite,'typ_op'=>$typ_op, 'user'=>$user,  'h'=>$h ' ));
