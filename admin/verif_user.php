...
if( $_SERVER['REQUEST_METHOD'] == "POST" ) {
	...
	if ((isset ($nom) && !empty($nom)) && (isset ($pass) &&!empty($pass)) && (isset ($prenom) && !empty($prenom)) && (isset ($fonc)))
	{
		...
		$fonc=htmlspecialchars(strtoupper($fonc));
			...
			try
			{
					...
					$req->execute(array('lognom'=>$nom, 'u_prenom'=>$prenom,'password'=>sha1($pass), 'fonct'=>$fonc)); 
